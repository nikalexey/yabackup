        ...
        def parseContent(content):
            ...
            for response in root.findall('.//d:response', namespaces=self.namespaces):
                node = {
                    ...
                    'isDir': response.find("d:propstat/d:prop/d:resourcetype/d:collection", namespaces=self.namespaces) is not None
                }
                if not node['isDir']:
...
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    def mkdirs(self, path):
        ...
        folders = self._splitFolder(path)
        path = ""
